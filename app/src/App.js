        ...
        <div className="buttons-container">
          {!this.state.webcamCapture && (
            <button
              ...
              onClick={this.takeSnaphotFromWebcam}
            >
              Capture Photo{" "}
              <img
          ...
          {this.state.webcamCapture && (
            <button
              ...
              onClick={() => this.setState({ webcamCapture: null })}
            >
              Retake Photo{" "}
              <img
                style={{ marginLeft: "10px" }}
          ...
          {this.state.webcamCapture && (
            <button className="upload-file__button" onClick={this.usePhoto}>
              Use this Photo{" "}
              <img
                style={{ marginLeft: "10px" }}
                height="20px"
    ...
    return (
      <div>
        <div className="app__container">
          {this.state.webcamVideoSelected && !this.state.photoUsed && (
            <>{this.renderWebcamVideoCapture()}</>
          )}
          {this.state.webcamSelected && !this.state.photoUsed && (
            <>{this.renderWebcamCapture()}</>
          )}
          {!this.state.webcamSelected && !this.state.webcamVideoSelected && (
            <div>
              <Header />
              <BodyContainer
                webcamCapture={this.state.webcamCapture}
                webcamCaptureBinary={this.state.webcamCaptureBinary}
                startWebcam={this.startWebcam}
                startWebcamVideo={this.startWebcamVideo}
              />
            </div>
          )}
